...
PROVIDERS = {
    "OpenAI": {"models": ["gpt-4o", "gpt-4-turbo", "gpt-3.5-turbo"], "base_url": None},
    "Groq (Llama/Mixtral)": {
...
with col_llm:
    ...
    if not user_api_key:
        ...
        if prompt := st.chat_input("Expliquez-moi mes résultats..."):
            ...
            Ton rôle est d'interpréter le risque de diabète calculé par
            ...
            1. ANALYSE CROISÉE : Ne te contente pas de lister les données.
                Fais des liens (ex: Lien entre BMI élevé, HighBP et manque d'activité physique).
            2. PERSONNALISATION : Si le patient a des difficultés à marcher (DiffWalk),
                suggère des activités à faible impact (natation, chaise).
            3. PSYCHOLOGIE : Si MentHlth est élevé, souligne l'importance du bien-être mental
                dans la gestion métabolique.
            4. SÉCURITÉ : Tu DOIS toujours finir par : "Cette analyse automatique ne remplace
                pas un diagnostic médical. Veuillez consulter un professionnel de santé."
            5. INTERDICTION : Ne prescris JAMAIS de médicaments ni de dosages
                (ex: Insuline, Metformine).

            Réponds de manière structurée avec des puces, en français, sur un ton
                bienveillant mais professionnel.
            """

            try:
                client = OpenAI(api_key=user_api_key, base_url=PROVIDERS[provider_name]["base_url"])
